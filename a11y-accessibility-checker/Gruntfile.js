...
module.exports = (grunt) => {
  ...
  grunt.initConfig({
    ...
    concat: {
      license: {
        ...
        files: {
          'dist/a-11-y-accessibility-checker/plugin.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ],
          'dist/a-11-y-accessibility-checker/plugin.min.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ]
        }
    ...
    copy: {
      css: {
        files: [
          { src: [ 'CHANGELOG.txt', 'LICENSE.txt' ], dest: 'dist/a-11-y-accessibility-checker', expand: true }
        ]
      }
    },
  ...
  grunt.registerTask('version', 'Creates a version file', () => {
    grunt.file.write('dist/a-11-y-accessibility-checker/version.txt', BUILD_VERSION);
  });
